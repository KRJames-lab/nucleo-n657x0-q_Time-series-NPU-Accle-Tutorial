...
int main(void)
{
  ...
  /* Program HSLV_VDDIO3 OTP fuse so XSPI2 IO pads can run at 1.8V high-speed.
     Required for Macronix MX25UM51245G Octal DTR at ~200 MHz. This is a
     one-time irreversible programming; the function checks if already set. */
  if (OTP_Config() != 0) {
    Error_Handler();
  }
...
static void MX_XSPI2_Init(void)
{
  ...
  hxspi2.Init.MemorySize = HAL_XSPI_SIZE_512MB;  /* MX25UM51245G = 512 Mbit = 64 MB */
...
/**
  ...
  *         Sets HSLV_VDDIO3 bit in BSEC_HW_CONFIG fuse so that XSPI2 I/O pads
  *         can operate at 1.8 V high-speed — required for Macronix MX25UM51245G
  *         Octal DTR operation on NUCLEO-N657X0-Q. OTP is ONE-TIME-PROGRAMMABLE;
  *         code first reads the fuse and only programs if not already set.
  * @retval 0 on success (or already programmed), negative on error
  */
static int32_t OTP_Config(void)
{
